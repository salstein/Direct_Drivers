...
class ResetPassword implements useCase<DataState<Response>, Param> {
  final PasswordResetRepository _passwordResetRepository;

  ResetPassword(this._passwordResetRepository);

  @override
  Future<DataState<Response>> execute({required Param params}) async{
    try {
      final response = await _passwordResetRepository.resetPassword(newPassword: params.newPassword!);
